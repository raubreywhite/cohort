...
cohorts with full provenance. Cohort construction stays strictly upstream
of analysis. The pipeline produces analytic data tables, then hands them
to whatever consumes them downstream.
...
first run the pipeline writes a snapshot to disk. On later runs it
restores the snapshot, and matching operations replay instantly with no
...
the cache key. Call `new_cohort("adults_female", from = "root", label =
"Adult women")` again, and the pipeline updates the label. It
invalidates nothing.
...
you change, the cache cannot detect the change. Either include a
version tag in `argset`, or call `cp$invalidate(cohort)` to drop the
cohort and force a recompute.
